...
export const updateExpense = async (req, res) => {
    try {
        ...
        const splitAmount = amount / totalMembers;
        const newSplitDetails = group.members.map((member) => ({
            user: member._id,
            amount: member._id.toString() === paidBy.toString() ? 0 : splitAmount, // Payer owes 0
        }));
...
export const getGroupBalances = async (req, res) => {
    try {
        ...
        group.members.forEach((member) => {
            group.members.forEach((otherMember) => {
                if (member._id !== otherMember._id) {
                    const amountUserOwes = actualBalances[member._id][otherMember._id] || 0;
                    const amountUserIsOwed = actualBalances[otherMember._id][member._id] || 0;
...
export const leaveExpense = async (req, res) => {
    try {
        ...
        if (expense.paidBy._id.toString() === userId) {
            res.status(400).json({
                success: false,
                message: "Person who paid cannot leave the expense. You can try deleting it",
            });
            return;
        }
